...
export class LuisDialog extends Dialog {
  static findTrigger(session: CallSession, intent: string): LuisDialog {
    const dialog = LuisDialog.intents
      .filter((x) => x.triggerOptions)
      .find((x) => x.triggerOptions.match === intent);
  ...
  static findCancel(session: CallSession, intent: string): LuisDialog {
    const dialog = LuisDialog.intents
      .filter((x) => x.cancelOptions)
      .find((x) => x.cancelOptions.match === intent);
